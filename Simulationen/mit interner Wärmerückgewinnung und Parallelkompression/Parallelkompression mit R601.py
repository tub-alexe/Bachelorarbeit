...
# Final Parameters
c22.set_attr(h=None, p=27.74)
GK.set_attr(ttd_l=10)
c24.set_attr(p=14.35)
c27.set_attr(p=None)
VD.set_attr(ttd_l=5)
...
param = list(np.linspace(27.7, 27.9, iterations))
param2 = list(np.linspace(14.3, 14.5, iterations2))
eta = []
p_gk = []
Hochdruck = []
Mitteldruck = []
...
for p1 in param:
    ...
    for p2 in param2:
        ...
        if ean.network_data.loc['epsilon'] < y:
            ...
            if nw.get_conn('21').get_attr('p').val == 35.0:
                Hochdruck += [x]
                Mitteldruck += [z]
                Wirkungsgrad += [y * 100]
                Tsink += [j]
...
param = list(np.linspace(27.74, 40, iterations))

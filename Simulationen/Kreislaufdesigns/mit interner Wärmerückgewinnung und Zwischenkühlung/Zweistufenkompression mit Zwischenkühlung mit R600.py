...
iterations = 60
iterations2 = 60
param = list(np.linspace(68.1, 68.5, iterations))
param2 = list(np.linspace(21.8, 22.2, iterations2))
eta = []
p_gk = []
Hochdruck = []
Mitteldruck = []
...
for p1 in param:
    ...
    for p2 in param2:
        ...
        if ean.network_data.loc['epsilon'] < y:
            ...
            if ean.component_data['E_D']['Zusammenführung'] < 0:
                Hochdruck += [x]
                Mitteldruck += [z]
                Wirkungsgrad += [y * 100]
                Tsink += [j]
...
param = list(np.linspace(53.4, 71.3, 80))

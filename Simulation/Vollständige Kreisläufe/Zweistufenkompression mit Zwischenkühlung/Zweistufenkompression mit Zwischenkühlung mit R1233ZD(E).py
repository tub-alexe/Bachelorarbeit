...
iterations = 60
iterations2 = 30
param = list(np.linspace(43.4, 43.8, iterations))
param2 = list(np.linspace(16.5, 16.6, iterations2))
...
for p1 in param:
    ...
    for p2 in param2:
        ...
        if ean.network_data.loc['epsilon'] < y:
            ...
            if nw.get_conn('21').get_attr('p').val == 38.0:
                Hochdruck += [x]
                Mitteldruck += [z]
                Wirkungsgrad += [y * 100]
                Tsink += [j]
...
param = list(np.linspace(38.4, 53, 100))

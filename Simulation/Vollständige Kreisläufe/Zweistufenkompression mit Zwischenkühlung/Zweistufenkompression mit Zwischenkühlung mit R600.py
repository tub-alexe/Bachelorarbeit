...
h_ihx_h_nach = CPSI("H", "P", 8.1 * 1e5, "T", 273.15+101, wf) * 1e-3
c1.set_attr(h=h_ihx_h_nach, p=8.1, fluid={'R600': 1, 'H2O': 0})


c1_co.set_attr(p=40)
h_c2 = CPSI("H", "P", 41 * 1e5, "T", 273.15+174, wf) * 1e-3
c2.set_attr(h=h_c2)


h_ihx_k_vor = CPSI("H", "P", 62 * 1e5, "T", 273.15+106, wf) * 1e-3
c4.set_attr(h=h_ihx_k_vor, p=62)

h_zw = CPSI("H", "P", 8.1 * 1e5, "Q", 1, wf) * 1e-3
c6_ue.set_attr(h=h_zw)

h_ihx_k_nach = CPSI("H", "P", 8.1 * 1e5, "T", 273.15+75, wf) * 1e-3
c7.set_attr(h=h_ihx_k_nach)

c8.set_attr(T=100, p=20, fluid={'R600': 0, 'H2O': 1})
c9.set_attr(T=200)

# Starting Parameters Connection Source
c10.set_attr(T=80, m=5, p=5, fluid={'R600': 0, 'H2O': 1})
c12.set_attr(T=75)
c13.set_attr(T=100, m=10, p=10, fluid={'R600': 0, 'H2O': 1})

#Solve Model
nw.solve(mode='design')
nw.print_results()

c1.set_attr(p=8.7965, h=None)
c1_co.set_attr(p= 40.6722)
ihx.set_attr(ttd_u=5)
c4.set_attr(h=None, p=50.2877, T=101.5257)

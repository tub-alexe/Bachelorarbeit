...
param = list(np.linspace(40.2, 40.8, iterations))
param2 = list(np.linspace(24.3, 24.6, iterations2))
eta = []
p_gk = []
Hochdruck = []
Mitteldruck = []
...
for p1 in param:
    ...
    for p2 in param2:
        ...
        # Wiederholtes Setzen von Startparametern, um stets stabile Startwerte zu gewährleisten
        ...
        if ean.network_data.loc['epsilon'] < y:
            ...
            if nw.get_conn('21').get_attr('p').val == 46.0:
                Hochdruck += [x]
                Mitteldruck += [z]
                Wirkungsgrad += [y * 100]
                Tsink_ein += [u]
            ...
            #Neuspeicherung der Variablen falls der optimale Zwischendruck noch nicht erreicht wurde
                ...
                u = nw.get_conn("13").get_attr("T").val
                v = nw.get_conn("14").get_attr("T").val
...
param = list(np.linspace(39, 50, 50))
